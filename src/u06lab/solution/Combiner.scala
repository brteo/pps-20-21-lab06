...
object Combiner{
  def apply[A](u: A, c: (A, A) => A): Combiner[A] = new Combiner[A](){
    override def unit:A = u
    override def combine(a: A, b: A): A = c(a, b)
  }

  implicit val SumCombiner = Combiner[Double](0.0, _ + _)
  implicit val ConcatCombiner = Combiner[String]("", _ + _)
  implicit val MaxCombiner = Combiner[Int](Int.MinValue, Math.max)
}
...
object FunctionsImpl extends Functions {
  ...
  def combine[A](a:Iterable[A])(implicit c: Combiner[A]): A = a.foldLeft(c.unit)(c.combine)
}
